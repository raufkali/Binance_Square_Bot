...
if (!LINKEDIN_CLIENT_ID) throw new Error("LINKEDIN_CLIENT_ID is missing.");
...
async function connectMongo() {
  ...
  try {
    ...
    await opportunitiesCollection.createIndex({ fetchedAt: -1 });
    await opportunitiesCollection.createIndex({ used: 1, fetchedAt: -1 });
...
/* =======================================================
   LINKEDIN TOKEN STORAGE (unchanged)
======================================================= */

async function getLinkedInToken() {
  if (!linkedinTokensCollection) return null;
  try {
    return await linkedinTokensCollection.findOne({ provider: "linkedin" });
  } catch {
    return null;
  }
}
...
async function handleLinkedInAuthCallback({ code, state, error }) {
  if (error) {
    return {
      statusCode: 400,
      html: `<!doctype html><html><body><h2>LinkedIn authorization failed</h2><p>${escapeHtml(error)}</p></body></html>`,
    };
  }
  if (!code || !consumeOAuthState(state)) {
    return {
      statusCode: 400,
      html: `<!doctype html><html><body><h2>Invalid OAuth request</h2><p>The authorization state is invalid or expired.</p></body></html>`,
    };
  }
  try {
    ...
    return {
      statusCode: 500,
      html: `<!doctype html><html><body><h2>LinkedIn connection failed</h2><p>${escapeHtml(error.message)}</p></body></html>`,
    };
  }
}
...
function inferOpportunityType(title, description, fallback) {
  ...
  if (/scholarship|scholarships|funded study|fully funded/.test(text))
    return "scholarship";
  if (/fellowship|fellowships/.test(text)) return "fellowship";
...
async function fetchRssSource(source) {
  ...
  try {
    const response = await fetchWithTimeout(
      ...
      {
        headers: {
          "User-Agent": "Mozilla/5.0 LinkedInOpportunityBot/4.0",
...
async function storeOpportunities(items) {
  ...
  const operations = items.map((item) => {
    ...
    return {
      updateOne: {
        filter: { fingerprint },
        update: { $setOnInsert: { ...item, fingerprint } },
        upsert: true,
      },
    };
  });
  try {
    await opportunitiesCollection.bulkWrite(operations, { ordered: false });
...
async function enrichOpportunity(opportunity) {
  ...
  if (!opportunity.url || !isRealUrl(opportunity.url)) return base;
  try {
...
- Description: ${opportunity.description || "Not available"}
...
function buildFinalPost(post, opportunity) {
  ...
  const org = post.organization.replace(/^#+/, "").replace(/\s+/g, " ").trim();
  const role = post.role.replace(/^#+/, "").replace(/\s+/g, " ").trim();

  let heading;
  if (post.type === "scholarship") heading = `${org} is OFFERING ${role}`;
  else if (post.type === "fellowship") heading = `${org} is OFFERING ${role}`;
  else if (post.type === "internship") heading = `${org} is OFFERING ${role}`;
  ...
  if (post.requirements.length) {
    ...
    for (const item of post.requirements) lines.push(`- ${item}`);
    lines.push("");
  }

  if (post.benefits.length) {
    lines.push("Benefits:");
    for (const item of post.benefits) lines.push(`- ${item}`);
    lines.push("");
  }

  if (post.eligibility.length) {
    lines.push("Eligibility:");
    for (const item of post.eligibility) lines.push(`- ${item}`);
    lines.push("");
  }
  ...
  if (isRealUrl(post.applicationUrl)) {
    ...
    lines.push("Check the source link in comments.");
  }
  lines.push("");
...
function buildImagePrompt(post) {
  ...
  switch (post.type) {
    case "scholarship":
      scene =
        "premium realistic photograph of an international university campus, modern academic buildings, students studying, bright professional academic environment";
      break;
    case "fellowship":
      scene =
        "premium realistic photograph of a modern research and professional fellowship environment, university research center, international collaboration atmosphere";
      break;
    case "internship":
      scene =
...
The opportunity is for: ${role}
Organization: ${organization}

Realistic professional photography. Landscape composition. Clean premium LinkedIn style. High detail. Natural lighting. No visible logos. No brand names. No readable text. No captions. No watermark.
`;
}
...
async function generateImageWithCloudflare(post) {
  ...
  const img = sharp(imageBuffer).resize(1200, 627, { fit: "cover" });
...
<svg width="1200" height="627">
  ...
  <rect x="0" y="0" width="1200" height="627" fill="url(#overlay)"/>
  <text x="70" y="115" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="bold" fill="white" letter-spacing="4">${escapeHtml(overlayText)}</text>
  <text x="70" y="185" font-family="Arial, Helvetica, sans-serif" font-size="52" font-weight="bold" fill="white">${escapeHtml(post.role.slice(0, 35))}</text>
  <text x="70" y="245" font-family="Arial, Helvetica, sans-serif" font-size="30" fill="white">${escapeHtml(post.organization.slice(0, 45))}</text>
</svg>
`;
...
async function createLinkedInPost(text, imageUrn = null) {
  const personUrn = await getLinkedInPersonUrn();
  if (!personUrn) throw new Error("No person URN found.");
  const body = {
    author: personUrn,
    commentary: text,
    visibility: "PUBLIC",
  ...
  if (imageUrn) {
    body.content = { media: { altText: "Career opportunity", id: imageUrn } };
  }
...
async function findOpportunity(opportunities) {
  ...
  for (const raw of shuffled) {
    ...
    if (recentTitles.includes(String(raw.title).toLowerCase().trim())) continue;
    return raw;
  }
  return null;
}
...
async function runCycle() {
  ...
  try {
    ...
    console.log(
      `🔗 Apply: ${post.applicationUrl || opportunity.finalUrl || opportunity.url || "Not provided"}`,
    );
    console.log("\n----- POST -----\n");
...
async function getUnusedDatabaseOpportunities() {
  ...
  try {
    ...
    return await opportunitiesCollection
      .find({ used: false, fetchedAt: { $gte: cutoff } })
      .sort({ fetchedAt: -1 })
      .limit(100)
      .toArray();
  } catch {
