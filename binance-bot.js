...
/*
=========================================================
BINANCE SQUARE AI BOT V10.3.1
TEXT ONLY – QUANTITATIVE CYCLE ANALYSIS (BTC ONLY)
=========================================================
*/
...
/* =======================================================
   TOPICS (still used for fallback and hashtags)
======================================================= */
...
/* =======================================================
   MARKET DATA – BTC ONLY
======================================================= */

async function getMarketData() {
  console.log("\n📊 [Binance] Fetching BTC market data (forced BTC mode)...");
  try {
    ...
    console.log(
      `   ✅ BTCUSDT $${formatPrice(lastPrice)} (${priceChange.toFixed(2)}%)`,
    );
    ...
    console.error("❌ BTC market data fetch failed:", error.message);
    return null;
  }
}
...
function movingAverage(data, period) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    result.push(window.reduce((total, value) => total + value, 0) / period);
  }
  return result;
}
...
/* =======================================================
   ADVANCED CYCLE METRICS (BTC ONLY)
======================================================= */
...
async function getAdvancedMarketData(symbol = "BTCUSDT") {
  ...
  if (!klinesRes.ok) throw new Error(`Weekly klines HTTP ${klinesRes.status}`);
  const klines = await klinesRes.json();
  ...
  if (weekly.length < 200) throw new Error("Insufficient weekly data.");
...
async function researchWeb() {
  ...
  try {
    const response = await fetchWithTimeout(GOOGLE_NEWS_URL, {
      headers: {
        "User-Agent": "Mozilla/5.0 BinanceSquareAI/10.3.1",
...
function normalizeGeneratedPost(post) {
  ...
  if (!allowedSignals.has(normalized.signal)) normalized.signal = "NONE";
...
function buildFallbackPost(
  ...
  let content;
  if (selectedTopic) {
    content = `$${tick} is at ${priceText}, with a 24h move of ${change.toFixed(2)}%. Current signal: ${signal}. ${reason} News angle: ${selectedTopic.title}.`;
  } else {
    content = `$${tick} is at ${priceText}, moving ${change.toFixed(2)}% over 24h. Current signal: ${signal}. ${reason} Topic: ${fallbackTopic}.`;
  }
...
function detectTopic(ticker, fallbackTopic) {
  ...
  if (MEME_COINS.includes(normalized)) return "meme";
  if (
...
`;
  ...
  // Advanced cycle data – always BTC
  let advancedBlock = "NO ADVANCED CYCLE DATA AVAILABLE.";
  try {
    const advanced = await getAdvancedMarketData("BTCUSDT");
    advancedBlock = buildTechnicalNarrative({ ...advanced, symbol: "BTCUSDT" });
  } catch (err) {
...
- Begin with "$BTC" (or the actual ticker, but always BTC).
...
- Do NOT skip just because there is no web research. Only skip if the market data or advanced data is completely missing.
...
function isDuplicate(post) {
  if (!post?.content) return { duplicate: false, score: 0 };
  const current = normalizeForDuplicate(post.content);
  if (!current) return { duplicate: false, score: 0 };
  ...
  for (const item of recent) {
    ...
    if (score >= 0.82) return { duplicate: true, score };
  }
  return { duplicate: false, score: highestScore };
}
...
async function runCycle() {
  resetDailyCounter();
  console.log("\n================================================");
  console.log("🚀 BINANCE SQUARE AI BOT V10.3.1");
  console.log("================================================");
  console.log(
  ...
  try {
    ...
    console.log(
      `   ✓ Duplicate protection passed. Highest similarity: ${duplicate.score.toFixed(2)}`,
    );
...
async function initializeBinanceBot() {
  ...
  console.log("🔥 Strategy: Quantitative Cycle + Market Analysis (BTC only)");
  console.log("🌐 Web research: Google News RSS");
  console.log("📊 Market data: Binance real-time (BTCUSDT)");
  console.log(
...
function getBinanceStatus() {
  ...
  return {
    ...
    imageModel: "None",
    advancedMetrics: "Enabled (BTC forced)",
  };
}
